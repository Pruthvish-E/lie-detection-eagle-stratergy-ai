...
def exploitation(model,X_train,y_train,k,rate):
	...
	#Iterations = 1000
	#maxs = present_weights[0][0]
	while(accuracy<first or accuracy<0.8):
		#for i in range(len(present_weights)):
		for i in range(len(present_weights)):
			if(len(present_weights[i].shape)==2):
				#print(present_weights[i][0],present_weights[i][0].size,present_weights[i][0][present_weights[i][0].size-1])
				for w in range(present_weights[i][0].size):
					if(random.randint(0,10)>5):
						present_weights[i][0][w]*=random.uniform(-100,100)
						present_weights[i][0][w]%=(10**9+7)
				for w in range(present_weights[i][1].size):
					if(random.randint(0,100)>98):
						present_weights[i][1][w]*=random.uniform(-100,100)
						present_weights[i][1][w]%=(10**9+7)
				

			else:
				for w in range(present_weights[i][0].size):
					if(random.randint(0,100)>98):
						present_weights[i][0]*=random.uniform(-100,100)
						present_weights[i][0]%=(10**9+7)
				
				#maxs = max(present_weights[i][j],maxs)
			#present_weights[i][j]%=(10**10+7)
	...
	print("train  -- - - - - - - - :- ",getAccuracy(model,X_train,y_train,present_weights))		
	return model
